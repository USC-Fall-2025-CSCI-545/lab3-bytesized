...
class AdaRRT():
    ...
    def __init__(self,
        ...
        self.goal_precision = goal_precision # epsilon
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            rand_sample = self._get_random_sample()
    ...
    def build_near_goal(self):
        """
        Build an RRT.

        In each step of the RRT:
            1. Sample a random point w.r.t p(random sample near goal) = 0.2 and p(random sample) = 0.8.
            2. Find its nearest neighbor.
            3. Attempt to create a new node in the direction of sample from its
                nearest neighbor.
            4. If we have created a new node, check for completion.
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        dist = self.dist(neighbor.state, sample)
    ...
    def _trace_path_from_start(self, node=None):
        ...
        curr_node = node if node != None else self.goal
        path = []
        while curr_node:
